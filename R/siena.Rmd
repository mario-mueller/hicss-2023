...
data <- sienaDataCreate(net, comp, devActVar,
                        depsUpVar, depsDownVar, comIntVar, relActVar, ageVar,
...
```{r}
# effects <- setEffect(effects, egoX, interaction="dev.ten", fix=TRUE, test=TRUE) 
# effects <-setEffect(effects, altX, interaction="bug.sev", fix=TRUE, test=TRUE)
```

```{r}
effects <- setEffect(effects, RateX, type="rate", name="net", interaction1="devActVar", test=TRUE, fix=TRUE)
ans.model1.stt <- siena07(algorithm, data=data, effects=effects,
                          batch=F, verbose=F, returnDeps=T, initC=T, useCluster=T,
                          nbrNodes=4)
outTable(ans.model1.stt)
```

```{r}
effects <- setEffect(effects, RateX, type="rate", name="net", interaction1="devActVar", include=TRUE, test=FALSE, fix=FALSE)
...
```{r}
effects <- setEffect(effects, inPop, name="net", test=TRUE, fix=TRUE)
ans.model2.stt <- siena07(algorithm, data=data, effects=effects,
                      batch=F, verbose=F, returnDeps=T, initC=T, useCluster=T,
                      nbrNodes=4)
outTable(ans.model2.stt)
```
...
( gof.id <- sienaGOF(ans.model0, verbose=TRUE, varName="net",
                     IndegreeDistribution, join=T, cumulative=F) )
plot(gof.id)

( gof.od <- sienaGOF(ans.model0, verbose=TRUE, varName="net",
