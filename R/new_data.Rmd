...
- TODO: Without comp, everything is good. Check with composition change
...
saveRDS(data, "data/sienaData_new_comp.rds")
print01Report(data, modelname='hicss-2023-new')
```
...
```{r}
algorithm <- sienaAlgorithmCreate(projname='hicss-2023-new-comp',
                                  seed=17,
...
When including composition change, outdegree distribution does not fit yet.
Maybe, change developer filter so that every developer is already involved in
the network at first observation, in line with how packages are handled...

```{r}
effects.fix <- effects

# Nope!
# effects.fix <- setEffect(effects.fix, outRateLog, name="net", type="rate",
#                          fix=F, test=F, include=T)
...
#                         fix=F, test=F, include=T)
# Good, but quite there yet
# effects.fix <- setEffect(effects.fix, outAct, fix=F, test=F, include=T)
# Gets better...

effects.fix <- setEffect(effects.fix, outInAss, fix=F, test=F, include=T)
...
texreg::htmlreg(list(model1, model2, model3, model4),
                ...
                custom.gof.rows = list(
                  "Wald Statistics" = c(gof.string1, gof.string2,
...
```{r}
sim <- ans.model10$sims[[400]][[1]][[1]][[3]]

g.layout <- gplot(sim, usearrows=T)
par(mfrow=c(1,1))
gplot(sim, gmode="twomode",
      arrowhead.cex=.75, edge.col='gray70', coord=g.layout, vertex.cex=1.5)
```

# Test With Composition Change
