...
```{r}
# Collaboration with same Owner
collab1 <- as.matrix(read.table('data/dyadic/collaborations-2021-02-01.txt'))
collab2 <- as.matrix(read.table('data/dyadic/collaborations-2021-03-01.txt'))
collab3 <- as.matrix(read.table('data/dyadic/collaborations-2021-04-01.txt'))

# Create network object
collabList  <- list(depDown1, depDown2, depDown3)
collabArray <- array(c(depDown1, depDown2, depDown3),
                     dim=c(num_devs, num_pkgs, length(depDownList)))
collabDyad  <- varDyadCovar(depDownArray, nodeSets = c("devs", "pkgs"), type = "bipartite")
```
...
effects5 <- setEffect(effects5, altX, name="net", interaction1="comIntLogVar",
                      fix=F, test=F, include=T)
effects5 <- setEffect(effects5, altX, name="net", interaction1="relActLogVar",
                      fix=F, test=F, include=T)
effects5 <- setEffect(effects5, altX, name="net", interaction1="ageLogVar",
                      fix=F, test=F, include=T)

model5 <- siena07(algorithm, data=data, effects=effects5,
                  batch=F, verbose=F, returnDeps=T, initC=T, useCluster=T,
                  byWave=T, nbrNodes=12)
summary(model5)

wald5 <- Multipar.RSiena(model5, c(4:model5$pp))

siena.table(model5, sig=T, type="html", file="tables/results_model5_final.html")
saveRDS(model5, "models/model5_final.rds")
```

```{r}
gof.id5 <- sienaGOF(model5, varName="net", IndegreeDistribution,
                    join=T, cumulative=F, verbose=T)
plot(gof.id5)

gof.od5 <- sienaGOF(model5, varName="net", OutdegreeDistribution,
                    join=T, cumulative=F, verbose=T)
plot(gof.od5)
```

In-degree distribution improves to p = 0.344.
Out-degree distribution p = 0.011, but still not quite there yet.
...
effects6 <- setEffect(effects6, altX, name="net", interaction1="depsUpLogVar",
                      fix=F, test=F, include=T)
effects6 <- setEffect(effects6, altX, name="net", interaction1="depsDownLogVar",
                      fix=F, test=F, include=T)

# effects6 <- setEffect(effects6, outActSqrt, fix=F, test=F, include=T)
# effects6 <- setEffect(effects6, outTrunc, parameter=2, fix=F, test=F, include=F)

model6 <- siena07(algorithm, data=data, effects=effects6,
                  batch=F, verbose=F, returnDeps=T, initC=T, useCluster=T,
                  byWave=T, nbrNodes=12)
summary(model6)

wald6 <- Multipar.RSiena(model6, c(4:model6$pp))

siena.table(model6, sig=T, type="html", file="tables/results_model6_final.html")
saveRDS(model6, "models/model6_final.rds")
```

```{r}
gof.id6 <- sienaGOF(model6, varName="net", IndegreeDistribution,
                    join=T, cumulative=F, verbose=T)
plot(gof.id6)

gof.od6 <- sienaGOF(model6, varName="net", OutdegreeDistribution,
                    join=T, cumulative=F, verbose=T)
plot(gof.od6)
```
